...
After completing the batch run, we parsed the timing data to determine the average runtime per cutoff and computed the speedup relative to the averaged serial baseline (`cutoff = 0`).  
The expected trend — consistent with our results — is a strong performance gain up to around **cutoff = 5**, after which further thread creation yields diminishing returns due to scheduling overhead and resource saturation.
